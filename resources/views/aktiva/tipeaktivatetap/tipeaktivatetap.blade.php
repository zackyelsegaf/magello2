...
@section('content')
                    ...
                    <div class="card rounded-default p-3 filterBox text-white">
                        <form method="GET" action="">
                            <div class="form-group mb-1">
                                ...
                                <input type="text" name="tipe_aktiva_tetap" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Nama Tipe Aktiva Tetap" value="{{ request('tipe_aktiva_tetap') }}">
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card card-table">
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover table-center mb-0" id="TipeAktivaTetapList">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#TipeAktivaTetapList').DataTable({
                processing: true,
                serverSide: true,
                ordering: true,
                searching: false,
                ajax: {
                    url: "{{ route('get-tipeaktivatetap-data') }}",
                    data: function(d) {
                        d.tipe_aktiva_tetap = $('input[name=tipe_aktiva_tetap]').val();
                    }
                },
                dom: "<'row'<'col-sm-12'B>>" +
                ...
                columns: [{
                    ...
                    {
                        data: 'tipe_aktiva_tetap',
                        name: 'tipe_aktiva_tetap',
                        orderable: true,
                        searchable: false
                    },
                    {
            ...
            $('#select_all').on('click', function() {
                $('.tipeaktivatetap_checkbox').prop('checked', this.checked);
            });

            $('#TipeAktivaTetapList tbody').on('mouseenter', 'tr', function() {
                $(this).css('cursor', 'pointer');
            });

            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.tipeaktivatetap_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
                ...
                if (selectedIds.length > 0) {
                    if (confirm('Apakah yakin ingin menghapus data yang dipilih?')) {
                        $.ajax({
                            url: "{{ route('tipeaktivatetap/delete') }}",
                            type: "POST",
                            data: {
                                ids: selectedIds,
                                _token: "{{ csrf_token() }}"
            ...
            $('#TipeAktivaTetapList tbody').on('click', 'tr', function(e) {
                ...
                var data = table.row(this).data();
                    if (data) {
                        window.location.href = "/tipeaktivatetap/edit/" + data.id;
                    }
            });
        });
    </script>

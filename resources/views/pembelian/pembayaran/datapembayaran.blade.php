    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <div class="mt-5">
                        <h4 class="card-title float-left mt-2">Data Pembayaran Pembelian</h4>
        ...
        <div class="row">
            <div id="filterBox" class="col-md-3" style="{{ request('filter') == '1' ? '' : 'display: none;' }}">
                <div class="card rounded-default p-3 filterBox  text-white">
                    <form method="GET" action="{{ route('pembelian/pembayaran/list/page') }}">
                        <input type="hidden" name="filter" value="1">
                        <div class="form-group mb-1">
                            <label>Pencarian</label>
                            <input type="text" name="no_pembayaran" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Cari berdasarkan ID" value="{{ request('no_pembayaran') }}">
                        </div>
                        <div class="form-group mb-1">
                            <input type="text" name="deskripsi_pembayaran" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Deskripsi" value="{{ request('deskripsi_pembayaran') }}">
                        </div>   
                        <div class="form-group mb-1">
                            <label>Pemasok</label>
                            <select class="form-control form-control-sm" name="pemasok_pembayaran" onchange="this.form.submit()">
                                <option value="" selected>-- Pemasok</option>
                                @foreach ($pemasok as $items)
                                    <option value="{{ $items->nama }}" {{ request('pemasok_pembayaran') == $items->nama ? 'selected' : '' }}>
                                        {{ $items->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-1">
                            ...
                            <select class="form-control form-control-sm" id="status_pembayaran" name="status_pembayaran" onchange="this.form.submit()">
                                <option value="">-- Semua Status --</option>
                                <option value="Menunggu" {{ request('status_pembayaran') == 'Menunggu' ? 'selected' : '' }}>Menunggu</option>
                                <option value="Diproses" {{ request('status_pembayaran') == 'Diproses' ? 'selected' : '' }}>Diproses</option>
                                <option value="Ditutup" {{ request('status_pembayaran') == 'Ditutup' ? 'selected' : '' }}>Ditutup</option>
                        ...
                        <div class="form-group mb-1">
                            ...
                            <select class="form-control form-control-sm" name="pengguna_pembayaran" onchange="this.form.submit()">
                                <option value="" selected>-- Pengguna</option>
                                @foreach ($pengguna_pembayaran as $items)
                                    <option value="{{ $items->pengguna_pembayaran }}" {{ request('pengguna_pembayaran') == $items->pengguna_pembayaran ? 'selected' : '' }}>
                                        {{ $items->pengguna_pembayaran }}
                                    </option>
                                @endforeach
                            </select>
                        </div> 
                ...
                <div class="card card-table">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover table-center mb-0" id="PembayaranList">
                    ...
                    <div class="mb-15 row">
                        <div class="col">
                            {{-- @if(Auth::user() && Auth::user()->email === 'user.zacky@gmail.com') --}}
                                <a href="{{ route('pembelian/pembayaran/add/new') }}" class="btn btn-primary float-left veiwbutton"><i class="fas fa-plus mr-2"></i>Tambah</a>
...
@section('script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#PembayaranList').DataTable({
                processing: true,
                serverSide: true,
                ordering: true,
                searching: false,
                ajax: {
                    url: "{{ route('get-pembayaran-data') }}",
                    data: function(d) {
                        d.no_pembayaran = $('input[name=no_pembayaran]').val(),
                        d.deskripsi_pembayaran = $('input[name=deskripsi_pembayaran]').val(),
                        d.pengguna_pembayaran = $('select[name=pengguna_pembayaran]').val(),
                        d.pemasok_pembayaran = $('select[name=pemasok_pembayaran]').val(),
                        ...
                        d.status_pembayaran = $('select[name=status_pembayaran]').val();
                    }
                },
                dom: "<'row'<'col-sm-12'B>>" +
                ...
                buttons: [
                    {
                        ...
                        title: 'Daftar Pembayaran Pembelian',
                        exportOptions: {
                ...
                columns: [{
                    ...
                    {
                        data: 'pemasok_pembayaran',
                        name: 'pemasok_pembayaran',
                        orderable: false,
                        searchable: false
                    },
                    {
                    ...
                    // },
                    {
                        data: 'pengguna_pembayaran',
                        name: 'pengguna_pembayaran',
                        orderable: false,
                        searchable: false
                    },
                    {
            ...
            $('#select_all').on('click', function() {
                $('.pembayaran_checkbox').prop('checked', this.checked);
            });

            $('#PembayaranList tbody').on('mouseenter', 'tr', function() {
                $(this).css('cursor', 'pointer');
            });

            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.pembayaran_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
                ...
                if (selectedIds.length > 0) {
                    if (confirm('Apakah yakin ingin menghapus data yang dipilih?')) {
                        $.ajax({
                            url: "{{ route('pembelian/pembayaran/delete') }}",
                            type: "POST",
                            data: {
                                ids: selectedIds,
                                _token: "{{ csrf_token() }}"
            ...
            $('#PembayaranList tbody').on('click', 'tr', function(e) {
                ...
                var data = table.row(this).data();
                    if (data) {
                        window.location.href = "/pembelian/pembayaran/edit/" + data.id + "/" + data.no_pembayaran;
                    }
            });
        });
    </script>

            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="card rounded-default p-3 filterBox text-white">
                        <form method="GET" action="{{ route('fasum/list/page') }}">
                            ...
                            <div class="form-group mb-1">
                                <label>Blok</label>
                                <select class="form-control form-control-sm" name="blok_fasum" onchange="this.form.submit()">
                                    <option value="" {{ request('blok_fasum') ? '' : 'selected' }}>-- Blok Fasum</option>
                                    @foreach ($fasum as $items)
                                        <option value="{{ $items->blok_fasum }}" {{ request('blok_fasum') === $items->blok_fasum ? 'selected' : '' }}>
                                        {{ $items->blok_fasum }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        $(document).ready(function() {
            var table = $('#DepartemenList').DataTable({
                ...
                columns:  [{
                    ...
                    {
                        data: 'nomor_unit_fasum',
                        name: 'nomor_unit_fasum',
                        orderable: false,
                        searchable: false
                    },
                    {
            ...
            $('#select_all').on('click', function() {
                $('.fasum_checkbox').prop('checked', this.checked);
            });
            ...
            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.fasum_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
            ...
            $('#DepartemenList tbody').on('click', 'tr', function(e) {
                ...
                var data = table.row(this).data();
                    if (data) {
                        window.location.href = "/fasum/edit/" + data.id;
                    }
            });
        });
    </script>

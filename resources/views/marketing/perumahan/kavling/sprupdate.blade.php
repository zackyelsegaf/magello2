...
@section('content')
    <div class="page-wrapper">
        <div class="content container-fluid">
            ...
            <form method="POST" action="{{ route('spr/update', $updateSPR->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <input type="hidden" name="booking_id" class="@error('booking_id') is-invalid @enderror" value="{{ old('booking_id', $updateSPR->booking_id) }}">
                    @error('booking_id')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                </div>
                <div class="mb-3">
                    <label for="nomor_spr" class="form-label fw-bold">No. SPR</label>
                    <input type="text" id="nomor_spr" class="form-control form-control-sm font-weight-bold" value="{{ $updateSPR->nomor_spr }}" readonly>
                </div>
                <div class="mb-3">
                    <label for="tanggal_pemesanan" class="form-label fw-bold">SPR dibuat pada tanggal :</label>
                    <input type="text" id="tanggal_pemesanan" name="tanggal_pemesanan" class="form-control form-control-sm datetimepicker @error('tanggal_pemesanan') is-invalid @enderror" value="{{ old('tanggal_pemesanan', $updateSPR->tanggal_pemesanan) }}">
                ...
                <div class="mb-3">
                    ...
                    <select id="lokasi_pemesanan" name="lokasi_pemesanan" class="tomselect @error('lokasi_pemesanan') is-invalid @enderror" placeholder="Ketik untuk cari!">
                        <option selected disabled {{ old('lokasi_pemesanan', $updateSPR->lokasi_pemesanan) ? '' : 'selected' }}>--Pilih Kota--</option>
                        @foreach ($kota as $k)
                            <option value="{{ $k->nama }}" {{ old('lokasi_pemesanan', $updateSPR->lokasi_pemesanan) == $k->nama ? 'selected' : '' }}>
                                {{ $k->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <div id="editor"></div>
                    <input type="hidden" name="deskripsi" id="dokumen_input" value="{{ old('deskripsi', $updateSPR->deskripsi) }}">
                </div>
                <div class="mb-4 row align-items-center">
                    <div class="col">
                        ...
                        <a href="{{ route('booking/list/page/pdf', $updateSPR->id) }}" class="btn btn-primary ml-2 buttonedit" target="_blank">
                            <i class="fas fa-file-pdf mr-2"></i> Cetak SPR
                        </a>
                    </div>
                </div>
            </form>

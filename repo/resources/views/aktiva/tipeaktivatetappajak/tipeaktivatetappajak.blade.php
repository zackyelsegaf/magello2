...
@section('content')
        ...
        <div class="content container-fluid">
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <div class="mt-5">
                            <h4 class="card-title float-left mt-2">Tipe Aktiva Tetap Pajak</h4>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card rounded-default p-3 filterBox text-white">
                        <form method="GET" action="">
                            <div class="form-group mb-1">
                                ...
                                <input type="text" name="tipe_aktiva_tetap_pajak" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Nama" value="{{ request('tipe_aktiva_tetap_pajak') }}">
                            </div>
                        </form>
                    </div>
                </div>
                <div class="col-md-9">
                    <div class="card card-table">
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover table-center mb-0" id="TipeAktivaTetapPajakList">
                    ...
                    <div class="page-header">
                        <div class="mb-15 row align-items-center">
                            <div class="col">
                                <a href="{{ route('tipeaktivatetappajak/add/new') }}" class="btn btn-primary float-left veiwbutton"><i
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#TipeAktivaTetapPajakList').DataTable({
                processing: true,
                serverSide: true,
                ordering: true,
                searching: false,
                ajax: {
                    url: "{{ route('get-tipeaktivatetappajak-data') }}",
                    data: function(d) {
                        d.tipe_aktiva_tetap_pajak = $('input[name=tipe_aktiva_tetap_pajak]').val();
                    }
                },
                dom: "<'row'<'col-sm-12'B>>" +
                ...
                columns: [{
                    ...
                    {
                        data: 'tipe_aktiva_tetap_pajak',
                        name: 'tipe_aktiva_tetap_pajak',
                        orderable: true,
                        searchable: false
                    },
                    {
            ...
            $('#select_all').on('click', function() {
                $('.tipeaktivatetappajak_checkbox').prop('checked', this.checked);
            });

            $('#TipeAktivaTetapPajakList tbody').on('mouseenter', 'tr', function() {
                $(this).css('cursor', 'pointer');
            });

            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.tipeaktivatetappajak_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
                ...
                if (selectedIds.length > 0) {
                    if (confirm('Apakah yakin ingin menghapus data yang dipilih?')) {
                        $.ajax({
                            url: "{{ route('tipeaktivatetappajak/delete') }}",
                            type: "POST",
                            data: {
                                ids: selectedIds,
                                _token: "{{ csrf_token() }}"
            ...
            $('#TipeAktivaTetapPajakList tbody').on('click', 'tr', function(e) {
                ...
                var data = table.row(this).data();
                    if (data) {
                        window.location.href = "/tipeaktivatetappajak/edit/" + data.id;
                    }
            });
        });
    </script>

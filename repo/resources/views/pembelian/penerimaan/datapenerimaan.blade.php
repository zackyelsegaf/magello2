    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <div class="mt-5">
                        <h4 class="card-title float-left mt-2">Data Penerimaan Barang</h4>
        ...
        <div class="row">
            <div id="filterBox" class="col-md-3" style="{{ request('filter') == '1' ? '' : 'display: none;' }}">
                <div class="card rounded-default p-3 filterBox text-white">
                    <form method="GET" action="{{ route('pembelian/penerimaan/list/page') }}">
                        <input type="hidden" name="filter" value="1">
                        <div class="form-group mb-1">
                            <label>Pencarian</label>
                            <input type="text" name="no_penerimaan" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Cari berdasarkan ID" value="{{ request('no_penerimaan') }}">
                        </div>
                        <div class="form-group mb-1">
                            <input type="text" name="deskripsi_penerimaan" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Deskripsi" value="{{ request('deskripsi_penerimaan') }}">
                        </div>   
                        <div class="form-group mb-1">
                            <label>Pemasok</label>
                            <select class="form-control form-control-sm" name="pemasok_penerimaan" onchange="this.form.submit()">
                                <option value="" selected>-- Pemasok</option>
                                @foreach ($pemasok as $items)
                                    <option value="{{ $items->nama }}" {{ request('pemasok_penerimaan') == $items->nama ? 'selected' : '' }}>
                                        {{ $items->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-1">
                            ...
                            <select class="form-control form-control-sm" id="status_penerimaan" name="status_penerimaan" onchange="this.form.submit()">
                                <option value="">-- Semua Status --</option>
                                <option value="Menunggu" {{ request('status_penerimaan') == 'Menunggu' ? 'selected' : '' }}>Menunggu</option>
                                <option value="Diproses" {{ request('status_penerimaan') == 'Diproses' ? 'selected' : '' }}>Diproses</option>
                                <option value="Ditutup" {{ request('status_penerimaan') == 'Ditutup' ? 'selected' : '' }}>Ditutup</option>
                        ...
                        <div class="form-group mb-1">
                            ...
                            <select class="form-control form-control-sm" name="pengguna_penerimaan" onchange="this.form.submit()">
                                <option value="" selected>-- Pengguna</option>
                                @foreach ($pengguna_penerimaan as $items)
                                    <option value="{{ $items->pengguna_penerimaan }}" {{ request('pengguna_penerimaan') == $items->pengguna_penerimaan ? 'selected' : '' }}>
                                        {{ $items->pengguna_penerimaan }}
                                    </option>
                                @endforeach
                            </select>
                        </div> 
            ...
            <div class="col-md-9">
                <div class="card card-table">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover table-center mb-0" id="PenerimaanList">
                ...
                <div class="page-header">
                    <div class="mb-15 row">
                        <div class="col">
                            <a href="{{ route('pembelian/penerimaan/add/new') }}" class="btn btn-primary float-left veiwbutton"><i class="fas fa-plus mr-2"></i>Tambah</a>
...
@section('script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#PenerimaanList').DataTable({
                processing: true,
                serverSide: true,
                ordering: true,
                searching: false,
                ajax: {
                    url: "{{ route('get-penerimaan-data') }}",
                    data: function(d) {
                        d.no_penerimaan = $('input[name=no_penerimaan]').val(),
                        d.deskripsi_penerimaan = $('input[name=deskripsi_penerimaan]').val(),
                        d.pengguna_penerimaan = $('select[name=pengguna_penerimaan]').val(),
                        d.pemasok_penerimaan = $('select[name=pemasok_penerimaan]').val(),
                        ...
                        d.status_penerimaan = $('select[name=status_penerimaan]').val();
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        title: 'Daftar Penerimaan Pembelian',
                        exportOptions: {
                            columns: ':not(:first-child):not(:nth-child(2))'
                        }
                    },
                ...
                columns: [{
                    ...
                    {
                        data: 'pemasok_penerimaan',
                        name: 'pemasok_penerimaan',
                        orderable: false,
                        searchable: false
                    },
                    {
                    ...
                    {
                        data: 'deskripsi_penerimaan',
                        name: 'deskripsi_penerimaan',
                        orderable: false,
                        searchable: false
                    },
                    {
                        data: 'pengguna_penerimaan',
                        name: 'pengguna_penerimaan',
                        orderable: false,
                        searchable: false
                    },
                    {
            ...
            $('#select_all').on('click', function() {
                $('.penerimaan_checkbox').prop('checked', this.checked);
            });

            $('#PenerimaanList tbody').on('mouseenter', 'tr', function() {
                $(this).css('cursor', 'pointer');
            });

            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.penerimaan_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
                ...
                if (selectedIds.length > 0) {
                    if (confirm('Apakah yakin ingin menghapus data yang dipilih?')) {
                        $.ajax({
                            url: "{{ route('pembelian/penerimaan/delete') }}",
                            type: "POST",
                            data: {
                                ids: selectedIds,
                                _token: "{{ csrf_token() }}"
            ...
            $('#PenerimaanList tbody').on('click', 'tr', function(e) {
                ...
                var data = table.row(this).data();
                    if (data) {
                        window.location.href = "/pembelian/penerimaan/edit/" + data.id + "/" + data.no_penerimaan;
                    }
            });
        });
    </script>
